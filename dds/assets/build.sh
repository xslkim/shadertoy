...
#        bash build.sh B10        # render only B10.html
set -u
cd "$(dirname "$0")"
shopt -s nullglob
CHROME="$(command -v chromium-browser || command -v chromium || command -v google-chrome || command -v chrome)"
if [ -z "$CHROME" ]; then
  echo "ERROR: no chrome/chromium found in PATH"; exit 1
fi
pat="${1:-}"
for f in B*.html; do
  base="${f%.html}"
